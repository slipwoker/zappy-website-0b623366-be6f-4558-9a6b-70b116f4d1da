  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, rule) {
      ...
      const messages = {
        ...
        minlength: 'Please enter at least ' + rule.value + ' characters.'
      };

      return messages[rule.type] || fieldName + ' is invalid.';
    }
    ...
    function showError(field, message) {
      ...
      let errorEl = document.getElementById('error-' + field.name);
      if (!errorEl) {
    ...
    function validateField(field) {
      ...
      if (isRequired && !validators.required(value)) {
        showError(field, getErrorMessage(field, { type: 'required' }));
        return false;
      }

      if (value.trim() !== '') {
        if (fieldType === 'email' && !validators.email(value)) {
          showError(field, getErrorMessage(field, { type: 'email' }));
          return false;
        }

        if (fieldType === 'tel' && !validators.tel(value)) {
          showError(field, getErrorMessage(field, { type: 'tel' }));
          return false;
        }

        if (field.minLength && field.minLength > 0) {
          if (!validators.minlength(value, field.minLength)) {
            showError(field, getErrorMessage(field, { type: 'minlength', value: field.minLength }));
            return false;
          }
        }
      }
    ...
    function validateForm() {
      ...
      fields.forEach(function (field) {
        if (field.type === 'submit' || field.type === 'button' || field.type === 'reset' || field.disabled) {
          return;
        }
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '0b623366-be6f-4558-9a6b-70b116f4d1da',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '0b623366-be6f-4558-9a6b-70b116f4d1da';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '0b623366-be6f-4558-9a6b-70b116f4d1da';
